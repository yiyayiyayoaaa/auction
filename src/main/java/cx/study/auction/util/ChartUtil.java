...
public class ChartUtil {
    public static JFreeChart createTimeSeriesByYear(String title, int year, List<CommodityStatistics> statistics){
        ...
        for (CommodityStatistics s: statistics){
            switch (s.getStatus()){
                case -1:
                    timeSeries = new TimeSeries("新增商品");
                    break;
    ...
    public static JFreeChart auctionWithMonth(String title, int month, List<CommodityStatistics> statistics){
        TimeSeriesCollection timeseriescollection = new TimeSeriesCollection();
        TimeSeries timeSeries = new TimeSeries(month+"月商品成交情况折线图");
        timeseriescollection.addSeries(timeSeries);
